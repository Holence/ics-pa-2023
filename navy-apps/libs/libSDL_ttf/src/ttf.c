...
int TTF_GlyphMetrics(TTF_Font *font, Uint16 ch, int *minx, int *maxx, int *miny, int *maxy, int *advance) {
  ...
  if (ret == 0)
    return -1;
  if (minx)
    *minx = fixedpt_toint(fixedpt_muli(font->factor, *minx));
  if (miny)
    *miny = fixedpt_toint(fixedpt_muli(font->factor, *miny));
  if (maxx)
    *maxx = fixedpt_toint(fixedpt_muli(font->factor, *maxx));
  if (maxy)
    *maxy = fixedpt_toint(fixedpt_muli(font->factor, *maxy));
