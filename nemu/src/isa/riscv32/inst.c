...
#define SHAMT BITS(imm, 4, 0)
  ...
  // mul
  INSTPAT("0000001 ????? ????? 000 ????? 0110011", mul, R, R(rd) = BITS((int64_t)(int32_t)src1 * (int64_t)(int32_t)src2, 31, 0));   // signed * signed [31:0]
  INSTPAT("0000001 ????? ????? 001 ????? 0110011", mulh, R, R(rd) = BITS((int64_t)(int32_t)src1 * (int64_t)(int32_t)src2, 63, 32)); // signed * signed [63:32]
  INSTPAT("0000001 ????? ????? 010 ????? 0110011", mulhsu, R, R(rd) = BITS((int64_t)(int32_t)src1 * (uint64_t)src2, 63, 32));       // signed * unsigned [63:32]
  INSTPAT("0000001 ????? ????? 011 ????? 0110011", mulhu, R, R(rd) = BITS((uint64_t)src1 * (uint64_t)src2, 63, 32));                // unsigned * unsigned [63:32]
  // div
  INSTPAT("0000001 ????? ????? 100 ????? 0110011", div, R, R(rd) = (int32_t)src1 / (int32_t)src2); // signed / signed
  INSTPAT("0000001 ????? ????? 101 ????? 0110011", divu, R, R(rd) = src1 / src2);                  // unsigned / unsigned
  // remainder
  INSTPAT("0000001 ????? ????? 110 ????? 0110011", rem, R, R(rd) = (int32_t)src1 % (int32_t)src2); // signed % signed
  INSTPAT("0000001 ????? ????? 111 ????? 0110011", remu, R, R(rd) = src1 % src2);                  // unsigned % unsigned
  // shift
  INSTPAT("0000000 ????? ????? 001 ????? 0110011", sll, R, R(rd) = src1 << src2);
  INSTPAT("0000000 ????? ????? 101 ????? 0110011", srl, R, R(rd) = src1 >> src2);
  INSTPAT("0100000 ????? ????? 101 ????? 0110011", sra, R, R(rd) = (int32_t)src1 < 0 ? ~(~src1 >> src2) : src1 >> src2); // shift right arithmetic
  // set less than
  INSTPAT("0000000 ????? ????? 010 ????? 0110011", slt, R, R(rd) = ((int32_t)src1 < (int32_t)src2) ? 1 : 0); // set less than (signed)
  INSTPAT("0000000 ????? ????? 011 ????? 0110011", sltu, R, R(rd) = (src1 < src2) ? 1 : 0);                  // set less than (unsigned)
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 0010011", srai, I, R(rd) = (int32_t)src1 < 0 ? ~(~src1 >> SHAMT) : src1 >> SHAMT); // shift right arithmetic
  // set less than
  INSTPAT("??????? ????? ????? 010 ????? 0010011", slti, I, R(rd) = ((int32_t)src1 < (int32_t)imm) ? 1 : 0); // set less than (signed)
  INSTPAT("??????? ????? ????? 011 ????? 0010011", sltiu, I, R(rd) = (src1 < imm) ? 1 : 0);                  // set less than (unsigned)
  // load and sign-extend
  INSTPAT("??????? ????? ????? 000 ????? 0000011", lb, I, R(rd) = SIGN_EXTEND(Mr(src1 + imm, 1), 8));
  INSTPAT("??????? ????? ????? 001 ????? 0000011", lh, I, R(rd) = SIGN_EXTEND(Mr(src1 + imm, 2), 16));
  INSTPAT("??????? ????? ????? 010 ????? 0000011", lw, I, R(rd) = SIGN_EXTEND(Mr(src1 + imm, 4), 32));
  // load and zero-extend
  INSTPAT("??????? ????? ????? 100 ????? 0000011", lbu, I, R(rd) = Mr(src1 + imm, 1));
  INSTPAT("??????? ????? ????? 101 ????? 0000011", lbh, I, R(rd) = Mr(src1 + imm, 2));
  ...
  // INSTPAT("0000000 00000 00000 000 00000 1110011", ecall, N, NEMUTRAP(s->pc, R(10))); // ebreak??
