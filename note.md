...
>
> - 对于批处理系统来说，基本没问题（除了暴露`halt()`就会允许某个程序直接当nemu关机）
> - 而对于多进程并发的系统来说，会有问题。如果直接把硬件的buffer暴露给用户程序，各个进程并发时会不顾对方的存在，交替覆写。所以需要操作系统对某些不能共享的硬件资源进行保护，一段时间内只允许一个进程使用（窗口管理器需要管理谁遮住谁，打印机需要连续打印完一份文件，网络数据包必须发完一个进程的再发另一个进程的）
